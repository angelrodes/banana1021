%% This script calculates iteratively erosion rates and burial times from 10Be and 21Ne concentrations scaled to their catchment average production rates (samples.csv).
...
if max([numel(P10),numel(P21),numel(L)])>4
    warning('Too many production rates. (max=4)')
end

if range([numel(P10),numel(P21),numel(L)])>0
...
% Define half lives
l10=log(2)/1.387e6;
l21=log(2)/4.543e9/1000; % 1000 times earth age. See this https://angelrodes.wordpress.com/2022/01/25/are-he-3-and-ne-21-stable-isotopes/
...
for sample=1:numel(C10)
    ...
    N21=@(er,t) (P21(1)+P21(2)+P21(3)+P21(4))+...
        exp(-l21.*t).*(...
        P21(1)./(l21+density.*er./1e4./L(1)) +...
        P21(2)./(l21+density.*er./1e4./L(2)) +...
        P21(3)./(l21+density.*er./1e4./L(3)) +...
        P21(4)./(l21+density.*er./1e4./L(4)) ...
            );
    
    %% find solutions
    chisq=@(er,t) ((((N10(er,t)-C10(sample))./dC10(sample)).^2+((N21(er,t)-C21(sample))./dC21(sample)).^2));
    plotpoints=200;
    ...
    while convergence<1
        ...
        elseif count>1000
            ...
            chimatrix=chisq(ematrix,tmatrix);
            chis=sort(unique(chimatrix(:)));
            maxchi=max(chimatrix(:));
            warning(['No solution found for ' samplename{sample}])
            sel=(chimatrix<=maxchi);
            sel2=find(chimatrix==min(chimatrix(:)));
            sel2=sel2(1);
            solutione=[unique(min(ematrix(sel))),unique(max(ematrix(sel))),ematrix(sel2)];
...
%         % solutions
%         if solutione(1)>-1
%             plot(N10(ematrix(selmatrix),tmatrix(selmatrix)),N21(ematrix(selmatrix),tmatrix(selmatrix))./N10(ematrix(selmatrix),tmatrix(selmatrix)),...
%                 '.','Color',[0.5 0.5 0.5])
%         end
        
        % erosions
        ttemp=[0, logspace(0,8,plotpoints), Inf];
        for etemp=[0 0.01 0.1 1 10 100 1000 Inf]
            N10temp=N10(etemp,ttemp);
            N21temp=N21(etemp,ttemp);
            plot(N10temp,N21temp./N10temp,'-','Color',[0.5 0.5 0.5])
            sel=1;
            text(N10temp(sel),N21temp(sel)./N10temp(sel),[num2str(etemp) ' m/Ma'],...
                'Color',[0.5 0.5 0.5],'HorizontalAlignment','left','VerticalAlignment','top', 'Clipping', 'on')
        end
        
        % ages long
        etemp=[0,logspace(-2,7,plotpoints)];
        for ttemp=[0 0.5 1 2 3 4 5 10 20 100 1000]*1e6
            N10temp=N10(etemp,ttemp);
            N21temp=N21(etemp,ttemp);
            plot(N10temp,N21temp./N10temp,'-','Color',[0.5 0.5 0.5])
            sel=find(N10temp<N10(0,0)/100,1,'first');
            text(N10temp(sel),N21temp(sel)./N10temp(sel),[num2str(ttemp/1e6) ' Ma'],...
                'Color',[0.5 0.5 0.5],'HorizontalAlignment','left','VerticalAlignment','top', 'Clipping', 'on')
        end
...
%             N10temp=N10(etemp,ttemp);
%             N21temp=N21(etemp,ttemp);
%             plot(N10temp,N21temp./N10temp,':b')
%             sel=length(N10temp);
%             text(N10temp(sel),N21temp(sel)./N10temp(sel),[num2str(ttemp/1e3) ' ka'],...
...
%         end
    ...
    C21theta=C21(sample)+cos(theta)*dC21(sample);
    xaxissample=C10theta;
    yaxissample=C21theta./C10theta;
    plot(xaxissample,yaxissample,'Color','k','LineWidth',1)
    plot(C10(sample),C21(sample)/C10(sample),'*k')
    
    xlabel('[^{10}Be]')
    ylabel('[^{21}Ne] / [^{10}Be]')
    
    
    text(C10(sample),C21(sample)/C10(sample)+dC21(sample)/C10(sample),samplename{sample},'HorizontalAlignment','center')
    
    set(gca,'xscale','log')
    box on
    xlim([min(N10(0,0)/1000,min(C10-dC10)/2) N10(0,0)*2])
    ylim([0 max(sum(P21)/sum(P10),max(C21./C10))*5])
    ...
    if sample==1
        ...
        %             'P21sp(basin)' '	' 'P21mu(basin)' '	' 'P21sp(post-burial)' '	' 'P21mu(post-burial)' '	'...
